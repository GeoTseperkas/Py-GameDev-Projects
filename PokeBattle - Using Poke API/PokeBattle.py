...
class Pokemon(pygame.sprite.Sprite):
    ...
    def get_total_stats(self):
        ...
        for stat in stats:
            if stat['stat']['name'] == 'hp':
                total += stat['base_stat'] + self.level
    ...
    def use_potion(self):
        ...
        if self.num_potions > 0:
            ...
            # add 60 hp (but don't go over the max hp)
            self.current_hp += 60
...
level = 50
pokemons = [
    Pokemon('Charizard', level, 25, 50),
    Pokemon('Blastoise', level, 175, 50),
    Pokemon('Venusaur', level, 325, 50),
    Pokemon('Dragonite', level, 475, 50),
    Pokemon('Gyarados', level, 625, 50),
    Pokemon('Snorlax', level, 775, 50),
    Pokemon('Alakazam', level, 925, 50),

    Pokemon('Gengar', level, 25, 250),
    Pokemon('Arcanine', level, 175, 250),
    Pokemon('Machamp', level, 325, 250),
    Pokemon('Lapras', level, 475, 250),
    Pokemon('Onix', level, 625, 250),
    Pokemon('Magmar', level, 775, 250),
    Pokemon('Rhydon', level, 925, 250),

    Pokemon('Scyther', level, 25, 450),
    Pokemon('Kangaskhan', level, 175, 450),
    Pokemon('Nidoking', level, 325, 450),
    Pokemon('Poliwrath', level, 475, 450),
    Pokemon('Raichu', level, 625, 450),
    Pokemon('Exeggutor', level, 775, 450),
    Pokemon('Ninetales', level, 925, 450),

    Pokemon('Zapdos', level, 25, 650),
    Pokemon('Articuno', level, 175, 650),
    Pokemon('Moltres', level, 325, 650),
    Pokemon('Mew', level, 475, 650),
    Pokemon('Mewtwo', level, 625, 650),
    Pokemon('Pikachu', level, 775, 650),
    Pokemon('Ditto', level, 925, 650)
    ]
...
while game_status != 'quit':
            ...
            if game_status == 'select pokemon':
                ...
                for i in range(len(pokemons)):
                    ...
                    if pokemons[i].get_rect().collidepoint(mouse_click):
                        ...
                        # select 10 random pokemon from the list
                        random_pokemons = random.sample(pokemons, 10)

                        # assign the rival's pokemon
                        rival_pokemon = None
                        for pokemon in random_pokemons:
                            if pokemon != player_pokemon:
                                if rival_pokemon is None or pokemon.get_total_stats() > rival_pokemon.get_total_stats():
    ...
    if game_status == 'select pokemon':
        ...
        text_score = font.render("Get Ready for Battle! Choose your Pokemon :", True, black)
        text_rect_scr = text_score.get_rect()
        text_rect_scr.center = (480, 20)
        game.blit(text_score, text_rect_scr)
